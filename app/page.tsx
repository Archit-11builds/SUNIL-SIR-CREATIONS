...
const nav = [
  ...
  ['ai','Archit AI',Bot], ['feedback','Feedback',MessageSquare], ['settings','Settings',Settings],
] as const;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setStatus(safeRead('ac_status', {})); setNotes(safeRead('ac_notes', {})); setExams(safeRead('ac_exams', []));
  ...
  const go = (p: string) => { setPage(p); setSelected(null); setChapter(null); setMobile(false); window.scrollTo(0, 0); };
  ...
  const plan = useMemo(() => {
    ...
    exams.forEach(ex => {
      ...
      if (!s) return;
      const cs = s.chapters.filter(c => planned.includes(c.id) && status[c.id] !== 'done');
      if (!cs.length) return;
  ...
  const resourceCards = (s: Subject, c: Chapter) => (['NCERT','Syllabus','Sample Papers','PYQs','Question Bank','Marking Scheme','Model Answers','E-Resources'] as ResourceType[]).map(type => {
    ...
    return <div className="resource premiumResource" key={type}>
      ...
      {doc ? <button className="resourceButton" onClick={() => setViewer(doc)}>Open inside site <ChevronRight size={14}/></button> : <button className="resourceButton" onClick={() => notify('Resource unavailable')}>Unavailable</button>}
    </div>;
  });
  ...
  const Dashboard = () => <>
    <header className="top"><div><small>YOUR BOARD JOURNEY</small><h1>Good evening, {name || 'Student'} 👋</h1><p>Your exam plan, resources and progress in one place.</p></div><button onClick={() => setDark(!dark)}>{dark ? <Star/> : <Moon/>}</button></header>
    <section className="hero"><div><span>CLASS {className} STUDY COMMAND CENTRE</span><h2>Study smarter.<br/><i>Finish stronger.</i></h2><p>Build your date sheet, select chapters and let Archit Creations organise the work.</p><button className="primary" onClick={() => go('datesheet')}>Build my study plan <ChevronRight/></button></div><div className="ring"><b>{prog}%</b><small>complete</small></div></section>
    ...
    <section className="commandGrid">
      <div className="commandCard goalCard"><div className="commandIcon"><Target/></div><div className="commandMain"><div className="commandTop"><div><small>TODAY'S STUDY GOAL</small><h3>{goalDone}/{dailyMinutes} minutes</h3></div><b>{goalPercent}%</b></div><div className="goalBar"><i style={{width:`${goalPercent}%`}}/></div><div className="goalControls"><button onClick={()=>setDailyMinutes(v=>Math.max(30,v-15))}>−15</button><button onClick={()=>setDailyMinutes(v=>v+15)}>+15</button><button className="miniPrimary" onClick={()=>setTimerOpen(true)}><Clock3/> Focus</button></div></div></div>
      <div className="commandCard examCard"><div className="commandIcon"><CalendarDays/></div><div><small>NEXT EXAM</small>{nextExam ? <><h3>{nextExam.subject}</h3><p>{pretty(nextExam.date)} · <b>{days(todayDate,nextExam.date)} days left</b></p></> : <><h3>No exam added</h3><p>Add your date sheet to activate countdowns.</p></>}<button className="textAction" onClick={()=>go('datesheet')}>Manage date sheet <ChevronRight/></button></div></div>
    </section>
    <section className="featureStrip"><button onClick={()=>setPaletteOpen(true)}><Search/><span><b>Quick command</b><small>Press Ctrl + K anytime</small></span><kbd>⌘K</kbd></button><button onClick={()=>go('revision')}><Brain/><span><b>Revision mode</b><small>4-step active recall workflow</small></span><ChevronRight/></button><button onClick={()=>go('analytics')}><BarChart3/><span><b>Performance</b><small>{activityDays} active study days</small></span><ChevronRight/></button></section>
    <div className="quickTools"><button onClick={() => setTimerOpen(true)}><Clock3/><span><b>Focus Timer</b><small>25-minute study session</small></span><ChevronRight/></button><button onClick={() => go('study')}><Zap/><span><b>What should I study?</b><small>Use your automatic plan</small></span><ChevronRight/></button></div>
    <News/>
    <div className="head"><div><small>TODAY'S PLAN</small><h2>{todayPlan.length ? 'Study these today' : 'No plan yet'}</h2></div><button onClick={() => go('datesheet')}>Date sheet <ChevronRight/></button></div>
    {todayPlan.length ? <div className="list">{todayPlan.map(x => <div className="row" key={`${x.date}-${x.chapter}`}><div><b>{x.subject} · {x.chapter}</b><small>{x.task}</small></div><button onClick={() => markPlan(x)}>{completed.includes(`${x.date}|${x.subject}|${x.chapter}`) ? <CheckCircle2/> : <ChevronRight/>}</button></div>)}</div> : <div className="empty"><CalendarDays/><h3>Build your date sheet to start</h3><p>Select chapters and the tracker will distribute them across the days before each exam.</p><button className="primary" onClick={() => go('datesheet')}>Open planner</button></div>}
    <div className="head"><div><small>SUBJECTS</small><h2>Resources for every subject</h2></div><button onClick={() => go('syllabus')}>View all <ChevronRight/></button></div>
    <div className="grid four">{subjects.map(s => <button className="card" key={s.id} onClick={() => { setSelected(s); setPage('subject'); }}><strong>{s.icon}</strong><div><h3>{s.name}</h3><p>{s.chapters.length} chapters · PDFs · notes · practice</p></div><ChevronRight/></button>)}</div>
    <div className="head"><div><small>ACHIEVEMENTS</small><h2>Unlock your board badges</h2></div><button onClick={()=>go('analytics')}>View performance <ChevronRight/></button></div>
    ...
    <div className="dashboardProStrip"><div><small>BOARD READINESS</small><b>{boardScoreCalc}/100</b><span>Completion · consistency · focus · mistake mastery</span></div><div className="miniReadiness"><i style={{width:`${boardScoreCalc}%`}}/></div><button className="secondaryButton" onClick={()=>go('rewards')}>Open readiness <ChevronRight size={14}/></button></div>
  </>;

  const Syllabus = () => <><div className="pageHead"><small>CLASS {className} SYLLABUS</small><h1>Every subject. Every chapter.</h1><p>Search, open a chapter and use the integrated resource viewer.</p></div><input className="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search chapter or subject..."/>{subjects.map(s => { const filtered = s.chapters.filter(c => `${s.name} ${c.name}`.toLowerCase().includes(search.toLowerCase())); if (!filtered.length) return null; return <section className="subject" key={s.id}><div className="subjectTitle"><b>{s.icon}</b><div><h2>{s.name}</h2><small>{filtered.length} matching chapters</small></div></div>{filtered.map(c => <button className="chapterRow" key={c.id} onClick={() => openChapter(s, c)}><span className={`dot ${status[c.id] || 'todo'}`}></span><span>{c.name}</span><small>{status[c.id] === 'done' ? 'Completed' : status[c.id] === 'progress' ? 'In progress' : 'Not started'}</small><ChevronRight size={15}/></button>)}</section>; })}</>;

  const SubjectPage = () => {
    if (!selected) return <Syllabus/>;
    return <><div className="pageHead"><small>{selected.name.toUpperCase()} RESOURCES</small><h1>{selected.icon} {selected.name}</h1><p>Integrated textbook, practice and revision resources for every chapter.</p></div>{chapter ? <><button className="back" onClick={() => setChapter(null)}>← Back to {selected.name}</button><div className="chapterHero"><div><small>CHAPTER WORKSPACE</small><h1>{chapter.name}</h1><p>{chapter.note}</p></div><button className={`status ${status[chapter.id] || 'todo'}`} onClick={() => cycle(chapter.id)}>{status[chapter.id] === 'done' ? '✓ Completed' : status[chapter.id] === 'progress' ? '● Work in Progress' : '○ Not Started'}</button></div><div className="resources">{resourceCards(selected, chapter)}<div className="resource"><b>Marking Scheme</b><p>Check how CBSE expects answers to be evaluated.</p><button className="resourceButton" onClick={()=>setViewer({title:`${selected.name} · CBSE Marking Scheme`,url:cbseMarkingScheme,description:'Official CBSE marking-scheme hub.'})}><FileText size={14}/> Open inside site <ChevronRight size={14}/></button></div><div className="resource"><b>Question Bank</b><p>Official CBSE question-bank hub for extra practice.</p><button className="resourceButton" onClick={()=>setViewer({title:`${selected.name} · CBSE Question Bank`,url:cbseQuestionBank,description:'Official CBSE question-bank hub.'})}><Library size={14}/> Open inside site <ChevronRight size={14}/></button></div></div><div className="notes"><h2>Your chapter notes</h2><textarea value={notes[chapter.id] || ''} onChange={e => setNotes(n => ({ ...n, [chapter.id]: e.target.value }))} placeholder="Write your revision notes..."/></div></> : <div className="grid">{selected.chapters.map(c => <button className="card chapterCard" key={c.id} onClick={() => setChapter(c)}><div><small className={`dot ${status[c.id] || 'todo'}`}></small><h3>{c.name}</h3><p>{c.note}</p><small>📖 PDF · 📝 Notes · ❓ Questions</small></div><ChevronRight/></button>)}</div>}</>;
  };
    ...
    return <>
      ...
      <section className="trackerSection chapterPlanner" id="chapter-planner">
        ...
        {activeExam && activeSubject ? <>
          ...
          <div className="plannerFooter"><span><b>{activeSelected}</b> chapters selected for {activeSubject.name}</span><button className="primary" onClick={() => document.getElementById('automatic-tracker')?.scrollIntoView({behavior:'smooth',block:'start'})}>Build my daily plan <ChevronRight/></button></div>
        </> : <div className="empty"><BookOpen/><h3>Your subject chapters will appear here</h3><p>Select an exam above. Archit Creations will keep every other subject out of this view so your tracker stays clean.</p></div>}
      </section>
      ...
      <section className="trackerSection" id="automatic-tracker">
        ...
        <div className="list">{plan.length ? plan.map(x => <div className="row" key={`${x.date}-${x.subject}-${x.chapter}`}><div><b>{x.date === todayDate ? 'TODAY' : pretty(x.date)} · {x.subject}</b><small>{x.chapter} — {x.task}</small></div><button onClick={() => markPlan(x)}>{completed.includes(`${x.date}|${x.subject}|${x.chapter}`) ? <CheckCircle2/> : <ChevronRight/>}</button></div>) : <div className="empty"><Zap/><h3>Your tracker is waiting for your selections</h3><p>Add an exam, choose that exam above, select its chapters, and your daily schedule will be generated here.</p></div>}</div>
      </section>
    </>;
  };

  const Study = () => <><div className="pageHead"><small>SMART STUDY ENGINE</small><h1>What should I study?</h1><p>Your next task is based on your exam dates, selected chapters and unfinished work.</p></div>{next ? <div className="recommend"><span>HIGH PRIORITY</span><h2>{next.chapter}</h2><p>{next.subject} · {next.task} · {next.date === todayDate ? 'Today' : pretty(next.date)}</p><button className="primary" onClick={() => { const s = subjects.find(x => x.name === next.subject); const c = s?.chapters.find(x => x.name === next.chapter); if (s && c) openChapter(s, c); }}>Start chapter <ChevronRight/></button></div> : <div className="empty"><h2>No scheduled task</h2><p>Build your date-sheet plan first.</p><button className="primary" onClick={() => go('datesheet')}>Build plan</button></div>}<div className="panel smartRules"><h3>How the engine works</h3><p>It prioritises selected chapters that are unfinished and spreads them over the available days before each exam.</p></div></>;

  const QuizArena = () => {
    const q = quizQuestions[quizIndex];
    const quizSubjectObj = subjects.find(s=>s.name===quizSubject);
    return <>
      <div className="pageHead"><small>ACTIVE RECALL LAB</small><h1>Quiz Arena <span className="aiBadge">SMART PRACTICE</span></h1><p>Fast board-style practice with instant scoring. Use it after a chapter instead of just rereading notes.</p></div>
      {!quizStarted ? <>
        <div className="quizSetup panel">
          <div className="quizSetupHead"><div className="featureIcon indigo"><ClipboardCheck/></div><div><h2>Build a quiz</h2><p>Pick a chapter, difficulty and question count.</p></div></div>
          ...
          <div className="quizActions"><button className="primary" onClick={generateQuiz}><Sparkles/> Generate quiz</button><button className="secondaryButton" onClick={()=>{setAiPrompt('Create a ' + quizCount + '-question ' + quizDifficulty + '-level Class 10 CBSE quiz for ' + quizSubject + (quizChapter ? ' — ' + quizChapter : '') + '. Give one question at a time with the answer hidden until I respond.');go('ai');}}><Bot size={15}/> Generate with Archit AI</button></div>
        </div>
        <div className="featureTiles"><div className="panel"><Brain/><b>Active recall</b><small>Try before checking notes.</small></div><div className="panel"><Target/><b>Board focus</b><small>Questions follow Class 10 study patterns.</small></div><div className="panel"><TrendingUp/><b>Track mastery</b><small>Your score feeds your readiness dashboard.</small></div></div>
      </> : <div className="quizLive">
        <div className="quizProgress"><span>QUESTION {quizIndex+1} / {quizQuestions.length}</span><b>{quizScore} correct</b></div>
        <div className="panel quizQuestion"><div className="quizMeta"><span>{quizSubject} · {quizDifficulty}</span><span>{quizChapter || 'Chapter'}</span></div><h2>{q?.q}</h2><div className="optionGrid">{q?.options.map(o=><button key={o} className={quizAnswer===o?'selected':''} onClick={()=>setQuizAnswer(o)}>{o}</button>)}</div><button className="primary" onClick={answerQuiz}>{quizIndex+1===quizQuestions.length?'Finish quiz':'Next question'} <ChevronRight/></button></div>
      </div>}
      {quizScore>0 && !quizStarted && <div className="panel resultBanner"><div><small>LAST SCORE</small><strong>{quizScore}%</strong></div><p>Keep your score above 80% to mark this as a strong revision session.</p><button className="secondaryButton" onClick={()=>go('mistakes')}>Review mistakes</button></div>}
    </>;
  };
  ...
  const Rewards = () => <>
    ...
    <div className="levelHero"><div className="levelOrb"><Medal/></div><div><small>LEVEL {level}</small><h2>{xp} XP</h2><p>{500-(xp%500)} XP until the next level</p><div className="xpBar"><i style={{width:`${(xp%500)/5}%`}}/></div></div><button className="primary" onClick={()=>setCertificateOpen(true)}><FileBadge/> Certificate</button></div>
    <div className="achievementGrid">{achievements.map(a=><div className={a.on?'achievement unlocked':'achievement'} key={a.title}><span>{a.icon}</span><div><b>{a.title}</b><small>{a.text}</small></div><Award size={16}/></div>)}</div>
    <div className="dashboardProStrip"><div><small>BOARD READINESS</small><b>{boardScoreCalc}/100</b><span>Completion · consistency · focus · mistake mastery</span></div><div className="miniReadiness"><i style={{width:`${boardScoreCalc}%`}}/></div><button className="secondaryButton" onClick={()=>go('rewards')}>Open readiness <ChevronRight size={14}/></button></div>
    <div className="rewardGrid"><div className="panel"><Flame/><h3>Streak Freeze</h3><p>Keep one recovery token ready if you miss a study day.</p><b>{streakFreeze} available</b><button className="secondaryButton" onClick={()=>setStreakFreeze(v=>v+1)}>Earn practice token</button></div><div className="panel"><Gauge/><h3>Board readiness</h3><strong className="readinessNumber">{boardScoreCalc}/100</strong><p>Built from completion, consistency, focus and mistake mastery.</p></div><div className="panel"><Trophy/><h3>Next milestone</h3><p>{done<10?'Complete 10 chapters':xp<1000?'Reach 1000 XP':focusSessions<10?'Finish 10 focus sessions':'Keep your streak alive.'}</p><button className="secondaryButton" onClick={()=>go('study')}>Work toward it</button></div></div>
  </>;
  ...
  const StudyRooms = () => <>
    ...
    <div className="roomBuilder"><div className="panel"><Users/><h3>Create a room</h3><input className="search" value={roomName} onChange={e=>setRoomName(e.target.value)} placeholder="Room name, e.g. Sunday Science Sprint"/><button className="primary" onClick={createRoom}><Plus/> Create room</button></div><div className="panel"><UserRoundPlus/><h3>Join a room</h3><input className="search" value={roomCode} onChange={e=>setRoomCode(e.target.value.toUpperCase())} placeholder="5-character code" maxLength={5}/><button className="secondaryButton" onClick={joinRoom}>Join room</button></div></div>
    {roomCode&&<div className="panel roomActive"><div><small>ACTIVE ROOM</small><h2>{roomCode}</h2><p>Share this code with your study group.</p></div><button className="secondaryButton" onClick={copyRoom}><Copy/> Copy code</button></div>}
    <div className="head"><div><small>YOUR ROOMS</small><h2>Study sprints</h2></div><span className="badge">{rooms.length} rooms</span></div>
    {rooms.length?<div className="roomGrid">{rooms.map(r=><div className="panel roomCard" key={r.code}><div className="roomCode">{r.code}</div><h3>{r.name}</h3><p>{r.goal}</p><small>👥 {r.members} member{r.members===1?'':'s'}</small><button className="textAction" onClick={()=>{setRoomCode(r.code);notify('Room selected')}}>Open room <ChevronRight/></button></div>)}</div>:<div className="empty"><Users/><h3>No study rooms yet</h3><p>Create one for a focused revision sprint.</p></div>}
  </>;
  ...
  const Leaderboard = () => {
    const starter = [
      { name: 'Aarav', xp: 980 }, { name: 'Riya', xp: 860 }, { name: 'Kabir', xp: 720 },
      { name: 'Ananya', xp: 640 }, { name: 'Rohan', xp: 540 },
    ];
    const combined = [...starter, ...leaderFriends, ...(name ? [{ name, xp }] : [])];
    ...
    return <><div className="pageHead"><small>COMMUNITY • LIVE ON THIS DEVICE</small><h1>ARCHIT CREATIONS LEADERBOARD</h1><p>Your rank changes automatically as your XP changes. Add classmates below to create your own study leaderboard.</p></div>
      ...
      <div className="panel"><h3>➕ Add a study buddy</h3><p>Add classmates manually. Their score stays on this browser and can be updated whenever you want.</p><div className="builderForm"><input value={friendName} onChange={e=>setFriendName(e.target.value)} placeholder="Student name"/><input type="number" min="0" value={friendXp} onChange={e=>setFriendXp(e.target.value)} placeholder="XP"/><button className="primary" onClick={addLeaderboardFriend}>Add to leaderboard</button></div></div>
  ...
  const askAI = async () => {
    ...
    try {
      ...
      setAiResponse(error instanceof Error ? error.message : 'Archit AI is unavailable right now.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const AIPage = () => <>
    <div className="pageHead"><small>INTELLIGENT STUDY LAB</small><h1>Meet Archit AI <span className="aiBadge">BETA</span></h1><p>Your Class 10 study assistant for explanations, quizzes, revision and planning.</p></div>
    <div className="aiHero"><div className="aiOrb"><Bot/></div><div><small>ARCHIT AI</small><h2>Ask. Learn. Practise.</h2><p>Ask for a simple explanation, a quiz, a revision plan or help understanding a Class 10 concept.</p></div><span className="freePill">FREE-TIER READY</span></div>
    <div className="aiQuick">{['Explain quadratic equations simply','Quiz me on Electricity','Make a 30-minute revision plan','Give me 5 competency-based questions'].map(q => <button key={q} onClick={() => setAiPrompt(q)}><Sparkles size={15}/>{q}</button>)}</div>
    <div className="aiComposer panel"><div className="aiInputRow"><textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Ask Archit AI anything about your Class 10 studies..." onKeyDown={e => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') askAI(); }}/><button className="aiSend" onClick={askAI} disabled={aiLoading}>{aiLoading ? <span className="aiSpinner"/> : <Send size={18}/>}<span>{aiLoading ? 'Thinking…' : 'Ask AI'}</span></button></div><small className="muted">Ctrl + Enter to send · Never put your API key in page.tsx.</small></div>
    {aiResponse && <div className="panel aiResponse"><div className="aiResponseHead"><Bot size={18}/><b>Archit AI</b><span>Study assistant</span></div><div className="aiText">{aiResponse}</div></div>}
    <div className="aiCards"><div className="panel"><Sparkles className="aiAccent"/><h3>Explain like Class 10</h3><p>Turn difficult topics into short, clear explanations with examples.</p></div><div className="panel"><Brain className="aiAccent"/><h3>Practice mode</h3><p>Generate questions and use them with your existing chapter resources.</p></div><div className="panel"><Target className="aiAccent"/><h3>Plan with context</h3><p>Archit AI receives your study progress and next-exam context when you ask for a plan.</p></div></div>
  </>;

  const Feedback = () => <><div className="pageHead"><small>HELP US IMPROVE</small><h1>Feedback</h1><p>Rate your experience and tell Archit what would make this better.</p></div><div className="panel feedbackPanel"><h3>How would you rate ARCHIT CREATIONS?</h3><div className="stars" role="radiogroup" aria-label="Rating">{[1,2,3,4,5].map(i => <button type="button" key={i} className={i <= feedbackRating ? 'starButton active' : 'starButton'} onClick={() => setFeedbackRating(i)} aria-label={`${i} star${i > 1 ? 's' : ''}`}>★</button>)}</div><p className="ratingLabel">{feedbackRating ? `${feedbackRating}/5 · ${['','Needs work','Okay','Good','Great','Excellent'][feedbackRating]}` : 'Tap a star to rate'}</p><textarea value={feedback} onChange={e => { setFeedback(e.target.value); setFeedbackSent(false); }} placeholder="What should we improve?"/>{feedbackSent && <p className="success">✓ Saved. On this version, feedback is stored in your browser so it doesn't disappear.</p>}<button className="primary" onClick={submitFeedback}>Send feedback</button></div><div className="panel"><h3>Your feedback history</h3><p className="muted">This is your private local history. It will stay on this browser until a cloud feedback destination is connected.</p>{feedbackHistory.length ? <div className="feedbackHistory">{feedbackHistory.map((f,i)=><div className="feedbackItem" key={`${f.date}-${i}`}><div><span>{'★'.repeat(f.rating)}{'☆'.repeat(5-f.rating)}</span><small>{new Date(f.date).toLocaleString('en-IN')}</small></div><p>{f.text}</p></div>)}</div> : <div className="empty"><MessageSquare/><h3>No feedback submitted yet</h3><p>Your submitted ratings will appear here.</p></div>}</div></>;

  const SettingsPage = () => <><div className="pageHead"><small>ARCHIT CONTROL CENTRE</small><h1>Settings & backup</h1><p>Personalise your study engine, appearance and saved progress.</p></div><div className="settingsGrid"><div className="panel appearancePanel"><div className="settingIcon"><Moon/></div><h3>Appearance</h3><p>Choose the look of Archit Creations. Your choice is saved automatically on this device.</p><div className="themeChoices"><button className={!dark ? 'themeChoice active' : 'themeChoice'} onClick={()=>setDark(false)}><span>☀</span><b>Light</b><small>Classic ivory</small></button><button className={dark ? 'themeChoice active darkChoice' : 'themeChoice'} onClick={()=>setDark(true)}><span>☾</span><b>Dark</b><small>Forest midnight</small></button></div><div className="themeStatus">Currently using <b>{dark ? 'Dark mode' : 'Light mode'}</b></div></div><div className="panel"><div className="settingIcon"><SlidersHorizontal/></div><h3>Daily study target</h3><p>Choose how many focused minutes you want to aim for each day.</p><div className="rangeRow"><input type="range" min="30" max="300" step="15" value={dailyMinutes} onChange={e=>setDailyMinutes(Number(e.target.value))}/><b>{dailyMinutes} min</b></div></div><div className="panel"><div className="settingIcon"><DownloadCloud/></div><h3>Backup your data</h3><p>Export exams, chapter status, notes, plans and activity as a JSON backup.</p><button className="primary" onClick={exportBackup}><DownloadCloud/> Export backup</button><label className="secondaryButton backupImport">Restore backup<input type="file" accept="application/json,.json" onChange={importBackup}/></label></div><div className="panel"><div className="settingIcon"><RotateCcw/></div><h3>Reset timer</h3><p>Quickly return the focus timer to a 25/5 setup.</p><button className="secondaryButton" onClick={()=>{switchTimerMode('focus');notify('Timer reset to 25 minutes')}}>Reset focus timer</button></div><div className="panel"><div className="settingIcon"><Sparkles/></div><h3>Premium mode</h3><p>Animations, keyboard command palette, achievements, countdowns and smart planning are enabled.</p><span className="premiumPill">✦ ARCHIT PREMIUM UI</span></div></div></>;

  const Content = () => page === 'dashboard' ? <Dashboard/> : page === 'syllabus' ? <Syllabus/> : page === 'subject' ? <SubjectPage/> : page === 'datesheet' ? DateSheet() : page === 'study' ? <Study/> : page === 'notes' ? <Notes/> : page === 'revision' ? <Revision/> : page === 'quiz' ? <QuizArena/> : page === 'mistakes' ? <MistakeBook/> : page === 'analytics' ? <Analytics/> : page === 'leaderboard' ? <Leaderboard/> : page === 'rewards' ? <Rewards/> : page === 'heatmap' ? <Heatmap/> : page === 'resources' ? <ResourceLibrary/> : page === 'rooms' ? <StudyRooms/> : page === 'ai' ? <AIPage/> : page === 'settings' ? <SettingsPage/> : <Feedback/>;

  return <main className={`${dark ? 'app dark' : 'app'} ${compactMode ? 'compactMode' : ''}`}>
    {onboard && <div className="overlay"><div className="welcome"><div className="devotionalMark" aria-label="Mahakal"><span>ॐ</span><b>MAHAKAL</b></div><h2>ARCHIT CREATIONS</h2><h1>Let's build your <i>board journey.</i></h1><p>Enter your name and class to personalise your tracker.</p><input value={name} onChange={e => setName(e.target.value)} placeholder="Your name"/><div className="classes">{['9','10'].map(c => <button className={className === c ? 'selected' : ''} onClick={() => setClassName(c)} key={c}>Class {c}</button>)}</div><button className="primary full" onClick={save}>Let's begin <ChevronRight/></button><small>DESIGNED WITH ♥ BY ARCHIT<br/>BUILT FOR YOUR BOARD JOURNEY</small></div></div>}
    <aside className={mobile ? 'sidebar open' : 'sidebar'}><div className="brand"><b className="brandMark">A<span>✦</span></b><span>ARCHIT<br/>CREATIONS</span><button onClick={() => setMobile(false)}><X/></button></div><nav>{nav.map(([id,label,Icon]) => <button className={page === id ? 'active' : ''} key={id} onClick={() => go(id)}><Icon size={18}/>{label}</button>)}</nav><div className="sideBottom"><button onClick={() => setTimerOpen(true)}><Clock3 size={18}/> Focus Timer</button></div></aside>
    <section className="main"><header className="mobile"><button onClick={() => setMobile(true)}><Menu/></button><b>ARCHIT CREATIONS</b><span>{prog}%</span></header><div className="content"><div className="pageTransition" key={`${page}-${selected?.id ?? ""}-${chapter?.id ?? ""}`}>{Content()}</div></div><footer><div className="footerBrand"><span className="devotionalMark small"><span>ॐ</span><b>MAHAKAL</b></span><span>DESIGNED WITH <em>♥</em> BY ARCHIT</span></div><div>BUILT FOR YOUR BOARD JOURNEY</div></footer></section>

    {viewer && <div className="viewerOverlay"><div className="viewer"><header><div><small>INTEGRATED RESOURCE</small><h2>{viewer.title}</h2><p>{viewer.description}</p></div><div className="viewerActions"><a href={viewer.url} target="_blank" rel="noreferrer"><ExternalLink size={16}/> Open</a><a href={viewer.url} target="_blank" rel="noreferrer"><Download size={16}/> Download</a><button onClick={() => setViewer(null)}><X/></button></div></header><iframe src={`/api/resource?url=${encodeURIComponent(viewer.url)}`} title={viewer.title} /></div></div>}

    {timerOpen && <div className="overlay"><div className="timerModal"><button className="closeX" onClick={() => setTimerOpen(false)}><X/></button><small>FOCUS TIMER</small><h2>{timerMode === 'focus' ? 'Deep work' : 'Short break'}</h2><div className="timerCircle"><b>{timerLabel}</b><span>{timerRunning ? 'FOCUSING' : 'READY'}</span></div><div className="timerModes"><button className={timerMode === 'focus' ? 'selected' : ''} onClick={() => switchTimerMode('focus')}>25 min focus</button><button className={timerMode === 'break' ? 'selected' : ''} onClick={() => switchTimerMode('break')}>5 min break</button></div><div className="timerActions"><button className="primary" onClick={() => setTimerRunning(!timerRunning)}>{timerRunning ? 'Pause' : 'Start'} <ChevronRight/></button><button className="secondaryButton" onClick={resetTimer}><TimerReset size={15}/> Reset</button></div></div></div>}

    {loginOpen && <div className="overlay"><div className="welcome login"><button className="closeX" onClick={() => setLoginOpen(false)}><X/></button><User/><h2>ARCHIT ACCOUNT</h2><p>This local profile is already active as <b>{name || 'Student'}</b>.</p><div className="loginRow"><Lock size={16}/><span>Progress is stored in this browser.</span></div><button className="primary full" onClick={() => { setLoginOpen(false); notify('Profile is active'); }}>Continue</button><small>Cloud login can be connected later with Supabase/Firebase.</small></div></div>}

    <button className="floatingProfile" onClick={() => setLoginOpen(true)} aria-label="Open profile"><User size={18}/></button>
    {paletteOpen && <div className="overlay paletteOverlay" onMouseDown={()=>setPaletteOpen(false)}><div className="palette" onMouseDown={e=>e.stopPropagation()}><div className="paletteHead"><Search/><input autoFocus placeholder="Search Archit Creations..." onChange={e=>setSearch(e.target.value)}/><kbd>ESC</kbd></div><div className="paletteList">{[['dashboard','Dashboard','Open your command centre',LayoutDashboard],['datesheet','Date Sheet','Build exams and automatic plan',CalendarDays],['study','What to Study','Get your next priority task',Zap],['syllabus','Syllabus','Search every chapter',BookOpen],['revision','Revision','Open the four-step revision flow',Brain],['analytics','Analytics','Track completion and active days',BarChart3],['leaderboard','Leaderboard','See your XP ranking',Trophy],['ai','Archit AI','Ask, learn, practise and revise',Bot],['settings','Settings','Backup and personalise',Settings]].filter(x=>String(x[1]).toLowerCase().includes(search.toLowerCase())||String(x[2]).toLowerCase().includes(search.toLowerCase())).map(([id,label,desc,Icon])=>{ const PaletteIcon = Icon as React.ElementType; return <button key={String(id)} onClick={()=>quickAction(String(id))}><PaletteIcon/><span><b>{String(label)}</b><small>{String(desc)}</small></span><ChevronRight/></button>; })}</div><div className="paletteFoot"><span>TIP</span> Use <b>Ctrl + K</b> to open this command centre anywhere.</div></div></div>}
